...
The census aggregates, the sector shapefile, the contiguity matrices and the state maps are hosted
on Amazon S3, in the bucket `previsao-de-demanda-na-aps`, region `sa-east-1`, and are readable
without credentials. That bucket was unreachable for a period in September 2026 while the account
was suspended, so the two derived tables the scripts consume are also kept in this repository,
gzipped, and the scripts read those by default.
...
To retrieve an object from the bucket:
...
boundaries, which are too large for version control and are not redistributed here. Take them from
the bucket or from IBGE.
