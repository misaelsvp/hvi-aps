...
**Code.** `build_hvi.py` reads the 2022 Census aggregates by sector and writes the eighteen indicators, the four dimension scores and the index. `build_contiguity.py` reads the census sector shapefile and writes the queen contiguity neighbour lists, one set per state. `cluster_sectors.py` runs the aggregation described above and writes the sector to cluster assignment. `build_supplement.py` joins the two into a single table, one row per sector, carrying the state, the municipality, the cluster, the eighteen indicators, the four dimension scores and the index. `validate.py` computes the statistics reported for the index and for the resulting territories: internal consistency of each dimension and of the composite, the rank correlation against the municipal index, spatial autocorrelation for the municipal case study under queen contiguity with 999 conditional permutations, and the share of tract-level variance preserved between planning units, the dispersion of their populations and the reduction in their number. Each function carries the equation it implements. `recover_sector_cluster.py` rebuilds the sector to cluster assignment from the published state maps instead of recomputing it, by extracting the cluster polygons embedded in each HTML file and locating the centroid of every sector inside them; it is the route to take when the maps are the artifact at hand.

Interactive maps of the resulting clusters for every state are at https://misaelsvp.github.io/hvi-aps/

**Data.** The two derived tables the scripts consume, one row per census tract and the tract to cluster assignment, are kept in `data/` as gzipped CSV. The census aggregates, the tract boundaries and the contiguity matrices are larger and stay on Amazon S3; `data/README.md` says where.
